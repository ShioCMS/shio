...
shioharaApp.controller("ShPostTypeItemCtrl",["$scope","$http","$window","$stateParams","$state","$rootScope","shWidgetResource","shPostTypeResource",function(a,b,f,d,e,c,g,h){a.postTypeId=d.postTypeId;a.shPostType=null;a.shWidgets=g.query();c.$state=e;a.shPostType=h.get({id:a.postTypeId},function(b){a.shPostNewItem=angular.copy(a.shPostType);for(b=0;b<a.shPostNewItem.shPostTypeAttrs.length;b++)a.shPostNewItem.shPostTypeAttrs[b].value="Novo Valor"+b});a.postTypeSave=function(){a.shPostType.$update(function(){e.go("content.post-type-select")})}}]);
