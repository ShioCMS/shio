...
shioharaApp.controller("ShPostEditCtrl",["$scope","$http","$window","$stateParams","$state","$rootScope","shPostResource","shAPIServerService",function(a,b,g,d,c,f,e,h){a.channelId=null;a.postId=d.postId;a.breadcrumb=null;a.shSite=null;a.shPost=e.get({id:a.postId},function(){null!=a.shPost.shChannel&&(a.channelId=a.shPost.shChannel.id,a.$evalAsync(b.get(h.get().concat("/channel/"+a.channelId+"/path")).then(function(b){a.breadcrumb=b.data.breadcrumb;a.shSite=b.data.shSite})))});a.openPreviewURL=function(){if(null!=
a.shPost.shChannel)a.$evalAsync(b.get(h.get().concat("/channel/"+a.shPost.shChannel.id+"/path")).then(function(b){b="PT-CHANNEL-INDEX"==a.shPost.shPostType.name?h.server().concat("/sites/SampleSite/default/pt-br"+b.data.channelPath.replace(RegExp(" ","g"),"-")):h.server().concat("/sites/SampleSite/default/pt-br"+b.data.channelPath.replace(RegExp(" ","g"),"-")+a.shPost.title.replace(RegExp(" ","g"),"-"));g.open(b,"_self")}));else{var c="PT-CHANNEL-INDEX"==a.shPost.shPostType.name?h.server().concat("/sites/SampleSite/default/pt-br"+
response.data.channelPath.replace(RegExp(" ","g"),"-")):h.server().concat("/sites/SampleSite/default/pt-br/"+a.shPost.title.replace(RegExp(" ","g"),"-"));g.open(c,"_self")}};a.postEditForm="template/post/form.html";a.postDelete=function(){e["delete"]({id:a.shPost.id},function(){c.go("content",{},{reload:!0})})};a.postSave=function(){a.shPost.$update(function(){})}}]);
