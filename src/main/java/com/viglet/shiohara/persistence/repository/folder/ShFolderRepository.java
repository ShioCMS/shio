...
public interface ShFolderRepository extends JpaRepository<ShFolder, String> {
	...
	@Query("select new com.viglet.shiohara.bean.ShFolderTinyBean(f.id, f.name, f.position, f.date) from ShFolder f where f.shSite = ?1 and f.rootFolder = ?2")
	...
	@Query("select new com.viglet.shiohara.bean.ShFolderTinyBean(f.id, f.name, f.position, f.date) from ShFolder f where f.parentFolder = ?1")
