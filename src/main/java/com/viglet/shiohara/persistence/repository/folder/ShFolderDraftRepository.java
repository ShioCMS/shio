...
import com.viglet.shiohara.persistence.model.folder.ShFolderDraft;
...
public interface ShFolderDraftRepository extends JpaRepository<ShFolderDraft, String> {
	...
	@Query("select new com.viglet.shiohara.bean.ShFolderTinyBean(f.id, f.name, f.position, f.date) from ShFolderDraft f where f.shSite = ?1 and f.rootFolder = ?2")
	...
	@Query("select new com.viglet.shiohara.bean.ShFolderTinyBean(f.id, f.name, f.position, f.date) from ShFolderDraft f where f.parentFolder = ?1")
